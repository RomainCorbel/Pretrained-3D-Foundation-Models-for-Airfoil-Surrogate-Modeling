...
class ShapenetDataset(Dataset):
    ...
    def __init__(self, root, split, npoints=2500, classification=False, class_choice=None, 
                 image=False, normalize=True, shapenenet = False):
        ...
        for item in self.cat:
            ...
            if not shapenenet:
                for token in split_data:
                    pts_name = token.split('/')[-1] + '.pts'
                    if token.startswith(self.cat[item]) and pts_name not in pts_names:
                        pts_names.append(pts_name)
            else: 
                for token in split_data:
                    token_cat = token.split('/')[1]      # synset exact shape_data/03001627/7facccfa81369078a8930422448288ea
                    token_name = token.split('/')[-1]    # nom du fichier
                    if token_cat == self.cat[item]:
                        pts_names.append(token_name + '.pts')

...
def train(device, model, train_loader, optimizer, scheduler, criterion='MSE'):
    model.train()
    model.encoder.eval()   #  force encoder to stay frozen in behavior
...
def main(device, train_dataset, val_dataset, Net, hparams, path,
         criterion='MSE', reg=1, val_iter=10, name_mod='GraphSAGE', val_sample=False):
    Path(path).mkdir(parents=True, exist_ok=True)
    baseline_dir = "baselines/metrics/scarce"
    baseline_name = name_mod.replace("Global", "")
    baseline_path = osp.join(baseline_dir, baseline_name, "model")
    ...
    print("Trainable params:", get_nb_trainable_params(model))
    ...
    pbar_train = tqdm(range(num_epoch), position=0)
    ...
    if name_mod not in ('PointNet', 'MLP', 'PointNetGlobal', 'MLPGlobal'):
        ...
        for data in val_dataset:
            d = data.clone()
            d.edge_index = nng.radius_graph(
                x=d.pos,
                r=hparams['r'],
                loop=True,
                max_num_neighbors=int(hparams['max_neighbors'])
            ).contiguous().long()
            val_dataset_ready.append(d)
    else:
        val_dataset_ready = val_dataset
    for epoch in pbar_train:
        ...
        train_loss = train(device, model, train_loader, optimizer, scheduler=lr_scheduler, criterion=criterion)
        del train_loader
        ...
        if val_iter is not None and (epoch % val_iter == val_iter - 1 or epoch == 0):
            if val_sample:
                ...
                val_loader = DataLoader(val_dataset_ready, batch_size=1, shuffle=False)
                val_loss = test(device, model, val_loader, criterion)
                del val_loader
    ...
    if val_iter is not None and len(val_loss_list) > 0:
        val_epochs = [e for e in range(num_epoch)
                    if e == 0 or e % val_iter == val_iter - 1]
        fig_val, ax_val = plt.subplots(figsize=(20, 5))
        ax_val.plot(val_epochs, val_loss_list, label='Val loss (surface, p)')
    ...
    log_file = osp.join(path, name_mod + "_log.json")
